		...
		let table_check = make("td");
			...
			if(current_role == "admin"){
				...
				let edit_action = make("div");
					...
					edit_action.setAttribute("onclick",'loadItemToEdit("'+forms[index].subject_id+'")');
			...
			if(pageType == "trash"){
				...
				let remove_action = make("div");
					...
					remove_action.setAttribute("onclick","moveToTrashHelper('"+forms[index].subject_id+"')");
					action_div.appendChild(remove_action);
...
function loadItemToEdit(id){
	...
	let params = [{"name":"subject_id", "value": item_id}];
	
	qBuilder.sendQuery(openModal,"/get_subject_by_id",params);
	...
	function openModal(data){
		...
		if(res_data.type == "success"){
			...
			showModalContent('subject_modal');
			
			res_data = res_data.subject;
					
			_("subject_code").value = res_data.subject_code;
			_("subject_name").value = res_data.subject_name;
			_("units").value = res_data.units;
...
function saveItem(){
	...
	{
		"name": "subject_name",
		"value": _("subject_name").value,
	},
	{
		"name": "subject_code",
		"value": _("subject_code").value,
	},
	{
		"name": "units",
		"value": _("units").value,
	...
	if(saveMode == "edit"){
		...
		qBuilder.sendQuery(feedBackSaving,"update_subject", params);
		createDialogue("wait");
		return;
	}
	...
	qBuilder.sendQuery(feedBackSaving,"save_subject",params);
...
function feedBackSaving(){
	...
	function close(){
		closeModalContent("subject_modal");
	}
	localStorage.setItem("shouldReloadStudents","true");
...
function addNewSubject(){
	...
	_("subject_code").value = "";
	_("subject_name").value = "";
	_("units").value = "1";
...
function moveToTrash(confirmed = undefined,silent=false){
	...
	let itemvalue = [{"name":"subject_id", "value": idSelected}];
	qBuilder.sendQuery(feedBackRemoving,"/remove_subject",itemvalue);
}
